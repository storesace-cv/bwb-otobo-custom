...
sub SendNotification {
    ...
    # BWB customization: notification templates are shared across queues.
    # Resolve this marker to the HTML signature configured on the ticket queue,
    # so BWB and ZS Angola keep their own identities without duplicate templates.
    if (
        $Notification{Body}
        && $Notification{Body} =~ /OTOBO_BWB_QUEUE_SIGNATURE/
        )
    {
        ...
        if ( $Ticket{QueueID} ) {
            ...
            if ( $Queue{SignatureID} ) {
                ...
                $QueueSignature = $Signature{Text} || '';
            }
        }

        $Notification{Body} =~ s{
            <p>\s*(?:\x{2014}|-|&mdash;)?\s*
            &lt;OTOBO_BWB_QUEUE_SIGNATURE&gt;\s*</p>
        }{<div style="margin-top:24px">&lt;OTOBO_BWB_QUEUE_SIGNATURE&gt;</div>}gx;

        $Notification{Body} =~ s/&lt;OTOBO_BWB_QUEUE_SIGNATURE&gt;/$QueueSignature/g;
        $Notification{Body} =~ s/<OTOBO_BWB_QUEUE_SIGNATURE>/$QueueSignature/g;
    }

    # BWB customization: OTOBO's standard customer real-name marker identifies
    # the person who submitted the request.  Keep that meaning and provide a
    # separate marker for the actual customer company in agent notifications.
    if (
        $Notification{Body}
        && $Notification{Body} =~ /OTOBO_BWB_CUSTOMER_COMPANY/
        )
    {
        my %Ticket = $TicketObject->TicketGet(
            TicketID      => $Param{TicketID},
        ...
        my $CustomerCompanyName = $Ticket{CustomerID} || '-';

        if ( $Ticket{CustomerID} ) {
            my %CustomerCompany = $Kernel::OM->Get('Kernel::System::CustomerCompany')->CustomerCompanyGet(
                CustomerID => $Ticket{CustomerID},
            );
            $CustomerCompanyName = $CustomerCompany{CustomerCompanyName} || $Ticket{CustomerID};
        }

        $CustomerCompanyName = $Kernel::OM->Get('Kernel::System::HTMLUtils')->ToHTML(
    ...
    if ( $Param{Notification}->{ContentType} && $Param{Notification}->{ContentType} eq 'text/html' ) {
        ...
        # BWB visual identity: use exactly the same state palette as the
        # customer portal.  The foreground colour is paired explicitly to
        # preserve contrast in email clients without CSS support.
        my %TicketForTemplate = $TicketObject->TicketGet(
            TicketID      => $Param{TicketID},
            DynamicFields => 0,
            Silent        => 1,
        );
        ...
        my $StateColours = $StatePalette{ $TicketForTemplate{StateID} || 0 }
            || [ '#64748b', '#ffffff' ];
        ...
        $Notification{Body} = $LayoutObject->Output(
            ...
            Data         => {
                ...
                StateBackgroundColor => $StateColours->[0],
                StateTextColor       => $StateColours->[1],
            },
        );
    }
    ...
    if ( $Recipient{Type} eq 'Agent' ) {
        ...
        my $FromName  = $ConfigObject->Get('NotificationSenderName');

        # BWB customization: internal agent notifications must use the address
        # through which the customer originally contacted the helpdesk.  This is
        # especially important because bwb-in receives mail for two addresses.
        my %Ticket = $TicketObject->TicketGet(
            TicketID      => $Param{TicketID},
            DynamicFields => 0,
            Silent        => 1,
        ...
        if ( ( $Ticket{Queue} // '' ) eq 'bwb-in' ) {
            $FromName = 'Helpdesk - BWB';
...
SELECT adm.a_to
FROM article a
INNER JOIN article_data_mime adm ON adm.article_id = a.id
WHERE a.ticket_id = ? AND a.article_sender_type_id = 3
ORDER BY a.id ASC
SQL
                Bind  => [ \$Param{TicketID} ],
                Limit => 1,
            ...
            while ( my @Row = $DBObject->FetchrowArray() ) {
                my $OriginalRecipient = lc( $Row[0] // '' );
                if ( $OriginalRecipient =~ m{\b(helpdesk\@(?:bwb\.pt|storesace\.cv))\b} ) {
                    $FromEmail = $1;
                }
            }

            # Safe fallback for tickets created through the web or by an agent.
            if ( $FromEmail !~ m{\Ahelpdesk\@(?:bwb\.pt|storesace\.cv)\z} ) {
                $FromEmail = 'helpdesk@bwb.pt';
            }
        }
        elsif ( ( $Ticket{Queue} // '' ) eq 'zsangola-in' ) {
            $FromName  = 'Helpdesk - ZS Angola';
            $FromEmail = 'assistencia@zsa-softwares.com';
        }

        # send notification
        my $From = $FromName . ' <' . $FromEmail . '>';
        ...
        # Closure notifications already refer to the agent's closing article.
        # Send these directly instead of creating another system article in the
        # ticket.  Besides keeping the history clean, this prevents the new
        # ArticleCreate event from recursively triggering the same notification.
        if ( ( $Notification{Name} // '' ) =~ m{Fecho de ticket com anexos} ) {
            my $EmailObject = $Kernel::OM->Get('Kernel::System::Email');
            my $Sent = $EmailObject->Send(
            ...
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'info',
                Message  => "Sent customer '$Notification{Name}' notification directly to '$Recipient{UserEmail}'.",
            );
            return 1;
        }
