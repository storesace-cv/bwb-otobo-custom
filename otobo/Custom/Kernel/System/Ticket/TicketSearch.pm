...
sub _BWBOwnershipOriginalTicketSearch {
    my ( $Self, %Param ) = @_;
...
sub TicketSearch {
    ...
    return $Self->_BWBOwnershipOriginalTicketSearch(%Param)
        if !$Param{UserID}
        || $Kernel::OM->Get('Kernel::System::BWBAccess')->IsGlobalAdministrator( UserID => $Param{UserID} );

    my $RequestedResult = $Param{Result} || 'HASH';
    my $RequestedLimit  = $Param{Limit};
    my @TicketIDs = $Self->_BWBOwnershipOriginalTicketSearch(
        %Param,
        Result => 'ARRAY',
        Limit  => 100_000,
    );
    ...
    for my $TicketID (@TicketIDs) {
        next if !$Kernel::OM->Get('Kernel::System::BWBAccess')->TicketAccessCheck(
            UserID => $Param{UserID}, TicketID => $TicketID,
        );
        push @Allowed, $TicketID;
    }

    return scalar @Allowed if $RequestedResult eq 'COUNT';
    splice @Allowed, $RequestedLimit if $RequestedLimit && @Allowed > $RequestedLimit;
    return @Allowed if $RequestedResult eq 'ARRAY';

    my %Original = $Self->_BWBOwnershipOriginalTicketSearch(
        %Param,
        Result => 'HASH',
        Limit  => 100_000,
    );
    my %AllowedLookup = map { $_ => 1 } @Allowed;
    delete $Original{$_} for grep { !$AllowedLookup{$_} } keys %Original;
    return %Original;
}
