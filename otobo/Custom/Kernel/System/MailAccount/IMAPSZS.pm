...
package Kernel::System::MailAccount::IMAPSZS;
        ...
        for ( my $Messageno = 1; $Messageno <= $NOM; $Messageno++ ) {
            ...
            if ( $MessageSize > ( $MaxEmailSize * 1024 ) ) {
                ...
                if ( !@Lines ) {
                    ...
                    # Unknown ZS senders remain recoverable until they are converted.
                    # The message is copied to a dedicated IMAP folder, then removed
                    # from INBOX so it cannot create the same ticket again.
                    my $TargetFolder;
                    if ( $Return[1] ) {
                        my %FetchedTicket = $Kernel::OM->Get('Kernel::System::Ticket')->TicketGet(
                            TicketID => $Return[1], DynamicFields => 0, Silent => 1,
                        );
                        $TargetFolder = 'Helpdesk - Pendentes' if ( $FetchedTicket{Queue} || '' ) eq 'zs-postmaster';
                    }
                    if ($TargetFolder) {
                        my %Mailbox = map { $_ => 1 } $IMAPOperation->( 'mailboxes' );
                        $IMAPOperation->( 'create_mailbox', $TargetFolder ) if !$Mailbox{$TargetFolder};
                        if ( $IMAPOperation->( 'copy', $Messageno, $TargetFolder ) ) {
                    ...
                    if ( $Return[0] && ( !$TargetFolder || $MessageStatus ne 'Failed' ) ) {
                        $CommunicationLogObject->ObjectLog(
                            ObjectLogType => 'Connection',
                            Priority      => 'Debug',
                            Key           => 'Kernel::System::MailAccount::IMAP',
                            Value         => "Message '$Messageno' safely processed and marked for deletion from INBOX.",
                        );
                    }
