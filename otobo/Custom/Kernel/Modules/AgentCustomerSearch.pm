...
sub Run {
    ...
    if ( !$Self->{Subaction} || $Self->{Subaction} eq 'SearchCustomerUser' ) {
        ...
        for my $CustomerUserID ( sort keys %CustomerUserList ) {
            ...
            my %BWBUser = $CustomerUserObject->CustomerUserDataGet( User => $CustomerUserID );
            next CUSTOMERUSERID if !$Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerUserAccessCheck(
                UserID => $Self->{UserID}, CustomerUserLogin => $CustomerUserID,
            );
        ...
        for my $CustomerID ( sort keys %CustomerCompanyList ) {
            next CUSTOMERID if !$Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerAccessCheck(
