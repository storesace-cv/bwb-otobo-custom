...
sub Run {
    ...
    # ------------------------------------------------------------ #
        ...
        $Output .= $LayoutObject->Notify( Info => 'Foi enviado um convite para definir a palavra-passe.' )
            if ( $Notification && $Notification eq 'InviteSent' );
        $Output .= $LayoutObject->Notify( Info => 'Foi enviada a mensagem para repor a palavra-passe.' )
            if ( $Notification && $Notification eq 'ResetSent' );
        $Output .= $LayoutObject->Notify( Priority => 'Error', Info => 'Não foi possível enviar a mensagem. Confirme o endereço de email e a configuração de envio.' )
            if ( $Notification && $Notification eq 'InviteError' );
        $Output .= $Self->_Edit(
            Nav    => $Nav,
            Action => 'Change',
            Source => $Source,
    ...
    # ------------------------------------------------------------ #
        ...
        # Send a fresh one-time password link without saving any other form changes.
        my $BWBInviteMode = $ParamObject->GetParam( Param => 'BWBInviteMode' ) || '';
        if ( $BWBInviteMode eq 'invite' || $BWBInviteMode eq 'password-reset' ) {
            my $ID = $ParamObject->GetParam( Param => 'ID' ) || '';
            my %InviteUser = $CustomerUserObject->CustomerUserDataGet( User => $ID );
            if ( !%InviteUser || !$BWBAccessObject->CustomerUserAccessCheck( UserID => $Self->{UserID}, CustomerUserLogin => $ID ) ) {
                return $LayoutObject->FatalError( Message => 'Não tem autorização para enviar esta mensagem.' );
            }
            my $Sent = $InviteUser{UserEmail} && $Kernel::OM->Get('Kernel::System::BWBInvite')->CreateAndSend(
                Type   => 'customer',
                Mode   => $BWBInviteMode,
                Login  => $InviteUser{UserLogin},
                Email  => $InviteUser{UserEmail},
                Name   => "$InviteUser{UserFirstname} $InviteUser{UserLastname}",
                UserID => $Self->{UserID},
            );
            my $Notification = $Sent ? ( $BWBInviteMode eq 'invite' ? 'InviteSent' : 'ResetSent' ) : 'InviteError';
            return $LayoutObject->Redirect(
                OP => "Action=$Self->{Action};Subaction=Change;ID=$ID;Search=$Search;Nav=$Nav;Notification=$Notification"
            );
        }
        ...
        if (
            ...
            || !( $BWBAccessObject->CustomerAccessCheck( UserID => $Self->{UserID}, CustomerID => $GetParam{UserCustomerID} )
                || ( $GetParam{UserStoreID} && $BWBAccessObject->StoreAccessCheck( UserID => $Self->{UserID}, StoreID => $GetParam{UserStoreID} ) ) )
            )
        {
            return $LayoutObject->FatalError( Message => 'Não tem autorização para alterar este utilizador.' );
        }
...
sub _Overview {
    ...
    if ( $Param{Search} ) {
        ...
        if ( my $Allowed = $Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerIDsGet( UserID => $Self->{UserID} ) ) {
            ...
            for my $Login ( keys %ListAllItems ) {
                my %Data = $CustomerUserObject->CustomerUserDataGet( User => $Login );
                delete $ListAllItems{$Login} if !$Allowed{ $Data{UserCustomerID} // '' };
            }
        }
        ...
        if ( my $Allowed = $Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerIDsGet( UserID => $Self->{UserID} ) ) {
            my %Allowed = map { $_ => 1 } @{$Allowed};
            for my $Login ( keys %List ) {
                my %Data = $CustomerUserObject->CustomerUserDataGet( User => $Login );
                delete $List{$Login} if !$Allowed{ $Data{UserCustomerID} // '' };
            }
        }
        ...
        if (%List) {
            ...
            for my $ListKey ( keys %List ) {
                ...
                # BWB Stores: enrich the customer-user overview with the
                # customer company name and the assigned store name.
                $UserData{BWBStoreCustomerCompanyName} = '-';
                $UserData{BWBStoreName}                = '-';

                if ( $UserData{UserCustomerID} ) {
                    my %CustomerCompany = $Kernel::OM->Get('Kernel::System::CustomerCompany')->CustomerCompanyGet(
                        CustomerID => $UserData{UserCustomerID},
                    );
                    $UserData{BWBStoreCustomerCompanyName}
                        = $CustomerCompany{CustomerCompanyName} || $UserData{UserCustomerID};
                }

                if ( $UserData{UserStoreID} ) {
                    my %Store = $Kernel::OM->Get('Kernel::System::BWBStore')->StoreGet(
                        StoreID => $UserData{UserStoreID},
                    );
                    $UserData{BWBStoreName} = $Store{StoreName} || '-';
                }

                $BWBUserData{$ListKey} = \%UserData;
            }

            for my $ListKey (
                sort {
                    lc( $BWBUserData{$a}->{BWBStoreCustomerCompanyName} // '' )
                        cmp lc( $BWBUserData{$b}->{BWBStoreCustomerCompanyName} // '' )
                        || lc( $BWBUserData{$a}->{BWBStoreName} // '' )
                        cmp lc( $BWBUserData{$b}->{BWBStoreName} // '' )
                        || lc( $BWBUserData{$a}->{UserFullname} // '' )
                        cmp lc( $BWBUserData{$b}->{UserFullname} // '' )
                        || lc($a) cmp lc($b)
                } keys %BWBUserData
                )
            {
                my %UserData = %{ $BWBUserData{$ListKey} };
                next if !$Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerUserAccessCheck(
                    UserID => $Self->{UserID}, CustomerUserLogin => $ListKey,
                );
