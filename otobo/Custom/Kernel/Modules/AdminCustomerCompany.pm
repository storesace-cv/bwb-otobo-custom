...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'Change' ) {
        ...
        if ( !$BWBAccessObject->CustomerAccessCheck( UserID => $Self->{UserID}, CustomerID => $CustomerID ) ) {
            return $LayoutObject->FatalError( Message => 'Não tem autorização para aceder a este cliente.' );
        }
