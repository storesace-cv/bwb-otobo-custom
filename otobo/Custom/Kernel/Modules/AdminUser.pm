...
sub Run {
    ...
    # ------------------------------------------------------------ #
        ...
        my $UserID = $ParamObject->GetParam( Param => 'UserID' )
            ...
            || '';
        if (
            !$Kernel::OM->Get('Kernel::System::BWBAccess')->IsGlobalAdministrator( UserID => $Self->{UserID} )
            && $UserID != $Self->{UserID}
        ) {
            my $Responsible = $Kernel::OM->Get('Kernel::System::BWBAccess')->ResponsibleUserIDGet( UserID => $UserID );
            return $LayoutObject->FatalError( Message => 'Não tem autorização para gerir este agente.' )
                if !$Responsible || $Responsible != $Self->{UserID};
        }
        my %UserData = $UserObject->GetUserData(
            UserID        => $UserID,
            NoOutOfOffice => 1,
    ...
    # ------------------------------------------------------------ #
        ...
        my $BWBLevel = $ParamObject->GetParam( Param => 'BWBLevel' ) || 'responsible';
        my $BWBResponsibleUserID = $Self->{UserID};
        my @BWBCustomerIDs = $ParamObject->GetArray( Param => 'BWBCustomerIDs' );
        my @BWBStoreIDs = $ParamObject->GetArray( Param => 'BWBStoreIDs' );
        ...
        if (
            ...
            my $Responsible = $Kernel::OM->Get('Kernel::System::BWBAccess')->ResponsibleUserIDGet( UserID => $GetParam{UserID} );
            return $LayoutObject->FatalError( Message => 'Não tem autorização para alterar este agente.' )
                if !$Responsible || $Responsible != $Self->{UserID};
        }
        ...
        if ( !%Errors )
        {
            ...
            if ($Update) {
                ...
                if ( $BWBLevel eq 'collaborator' ) {
                    $AccessObject->HierarchySet(
                        UserID => $GetParam{UserID}, ResponsibleUserID => $BWBResponsibleUserID,
                        ChangeUserID => $Self->{UserID},
                    );
                    $AccessObject->CollaboratorAssignmentsSet(
                        UserID => $GetParam{UserID}, CustomerIDs => \@BWBCustomerIDs,
                        StoreIDs => \@BWBStoreIDs, ChangeUserID => $Self->{UserID},
                    );
                }
                else {
                    $AccessObject->HierarchyDelete( UserID => $GetParam{UserID} );
                    my $GroupID = $Kernel::OM->Get('Kernel::System::Group')->GroupLookup(
                        Group => 'bwb_customer_managers',
                    );
                    if ($GroupID) {
                        $Kernel::OM->Get('Kernel::System::Group')->PermissionGroupUserAdd(
                            GID => $GroupID, UID => $GetParam{UserID}, Permission => { rw => 1 },
                            UserID => $Self->{UserID},
                        );
                    }
                    $AccessObject->CustomersForOwnerSet(
                        OwnerUserID => $GetParam{UserID}, CustomerIDs => \@BWBCustomerIDs,
                        UserID => $Self->{UserID},
                    );
                }
        ...
        $Self->_Edit(
            ...
            %Errors,
            BWBLevel => $BWBLevel,
            BWBResponsibleUserID => $BWBResponsibleUserID,
            BWBCustomerIDs => \@BWBCustomerIDs,
            BWBStoreIDs => \@BWBStoreIDs,
        );
        $Output .= $LayoutObject->Output(
            TemplateFile => 'AdminUser',
            Data         => \%Param,
    ...
    # ------------------------------------------------------------ #
        ...
        my $BWBLevel = $ParamObject->GetParam( Param => 'BWBLevel' ) || 'responsible';
        $BWBLevel = 'collaborator'
            if !$Kernel::OM->Get('Kernel::System::BWBAccess')->IsGlobalAdministrator( UserID => $Self->{UserID} );
        my $BWBResponsibleUserID = $Self->{UserID};
        my @BWBCustomerIDs = $ParamObject->GetArray( Param => 'BWBCustomerIDs' );
        my @BWBStoreIDs = $ParamObject->GetArray( Param => 'BWBStoreIDs' );
        ...
        if ( !%Errors )
        {
            ...
            if ($UserID) {
                ...
                # Every newly created agent or collaborator may consult the
                # internal Helpdesk manuals, but cannot create or edit them.
                my $ManualReadersGroupID = $Kernel::OM->Get('Kernel::System::Group')->GroupLookup(
                    Group => 'bwb_helpdesk_manual_readers',
                );
                if ($ManualReadersGroupID) {
                    $Kernel::OM->Get('Kernel::System::Group')->PermissionGroupUserAdd(
                        GID        => $ManualReadersGroupID,
                        UID        => $UserID,
                        Permission => { ro => 1 },
                        UserID     => $Self->{UserID},
                    );
                ...
                $Kernel::OM->Get('Kernel::System::BWBInvite')->CreateAndSend(
                    Type => 'agent', Login => $GetParam{UserLogin}, Email => $GetParam{UserEmail},
                    Name => "$GetParam{UserFirstname} $GetParam{UserLastname}", UserID => $Self->{UserID},
                );

                my $AccessObject = $Kernel::OM->Get('Kernel::System::BWBAccess');
                if ( $BWBLevel eq 'collaborator' ) {
                    $AccessObject->HierarchySet(
                        UserID => $UserID, ResponsibleUserID => $BWBResponsibleUserID,
                        ChangeUserID => $Self->{UserID},
                    );
                    # Use Jorge's presentation preferences as the common visual
                    # standard. Access, queues, filters and widget positions are
                    # intentionally not copied by this method.
                    $AccessObject->VisualPreferencesCopy(
                        SourceUserID => 2,
                        TargetUserID => $UserID,
                    );
                    $AccessObject->CollaboratorAssignmentsSet(
                        UserID => $UserID, CustomerIDs => \@BWBCustomerIDs,
                        StoreIDs => \@BWBStoreIDs, ChangeUserID => $Self->{UserID},
                    );
                }
                else {
                    ...
                    if ($GroupID) {
                        $Kernel::OM->Get('Kernel::System::Group')->PermissionGroupUserAdd(
                            GID => $GroupID, UID => $UserID, Permission => { rw => 1 },
                            UserID => $Self->{UserID},
                        );
                    }
                    my $StatsGroupID = $Kernel::OM->Get('Kernel::System::Group')->GroupLookup( Group => 'stats' );
                    if ($StatsGroupID) {
                        $Kernel::OM->Get('Kernel::System::Group')->PermissionGroupUserAdd(
                            GID => $StatsGroupID, UID => $UserID, Permission => { rw => 1 },
                            UserID => $Self->{UserID},
                        );
                    }
                    $AccessObject->CustomersForOwnerSet(
                        OwnerUserID => $UserID, CustomerIDs => \@BWBCustomerIDs,
                        UserID => $Self->{UserID},
                    );
                }
...
sub _Edit {
    ...
    if ( $Param{Action} eq 'Change' ) {
        ...
        if ( $DBObject->Prepare(
            ...
            my ($ResponsibleID) = $DBObject->FetchrowArray();
            $Param{BWBResponsibleUserID} ||= $ResponsibleID;
            $Param{BWBLevel} ||= $ResponsibleID ? 'collaborator' : 'responsible';
        }

        my %Companies = $Kernel::OM->Get('Kernel::System::CustomerCompany')->CustomerCompanyList( Valid => 0 );
        my $CreatorCustomers = $Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerIDsGet( UserID => $Self->{UserID} );
        if ($CreatorCustomers) { my %Allowed = map { $_ => 1 } @{$CreatorCustomers}; delete $Companies{$_} for grep { !$Allowed{$_} } keys %Companies; }
        my $AccessObject = $Kernel::OM->Get('Kernel::System::BWBAccess');
        my $Assigned = $AccessObject->CustomerIDsGet( UserID => $Param{UserID} ) || [];
        $Assigned = $Param{BWBCustomerIDs} if ref $Param{BWBCustomerIDs} eq 'ARRAY';
        $Param{BWBCustomerOption} = $LayoutObject->BuildSelection(
            Data => \%Companies, Name => 'BWBCustomerIDs', SelectedID => $Assigned,
            Multiple => 1, Size => 8, Class => 'Modernize W50pc',
        );
        my $Stores = $Kernel::OM->Get('Kernel::System::BWBStore')->StoreList( IncludeInvalid => 0 );
        my $CreatorStores = $AccessObject->StoreIDsGet( UserID => $Self->{UserID} );
        if ($CreatorStores) { my %Allowed = map { $_ => 1 } @{$CreatorStores}; $Stores = [ grep { $Allowed{ $_->{StoreID} } } @{$Stores} ]; }
        my %StoreList = map { $_->{StoreID} => $_->{CustomerID} . ' | ' . $_->{StoreNumber} . ' - ' . $_->{StoreName} } @{$Stores};
        my $AssignedStores = $AccessObject->StoreIDsGet( UserID => $Param{UserID} ) || [];
        $AssignedStores = $Param{BWBStoreIDs} if ref $Param{BWBStoreIDs} eq 'ARRAY';
        $Param{BWBStoreOption} = $LayoutObject->BuildSelection(
            Data => \%StoreList, Name => 'BWBStoreIDs', SelectedID => $AssignedStores,
            Multiple => 1, Size => 8, Class => 'Modernize W50pc',
        );
    }

    if ( $Param{Action} eq 'Add' ) {
        $Param{BWBLevel} ||= 'responsible';
        my %Companies = $Kernel::OM->Get('Kernel::System::CustomerCompany')->CustomerCompanyList( Valid => 0 );
        my $CreatorCustomers = $Kernel::OM->Get('Kernel::System::BWBAccess')->CustomerIDsGet( UserID => $Self->{UserID} );
        if ($CreatorCustomers) { my %Allowed = map { $_ => 1 } @{$CreatorCustomers}; delete $Companies{$_} for grep { !$Allowed{$_} } keys %Companies; }
        my $Assigned = ref $Param{BWBCustomerIDs} eq 'ARRAY' ? $Param{BWBCustomerIDs} : [];
        $Param{BWBCustomerOption} = $LayoutObject->BuildSelection(
            Data => \%Companies, Name => 'BWBCustomerIDs', SelectedID => $Assigned,
            Multiple => 1, Size => 8, Class => 'Modernize W50pc',
        );
        my $Stores = $Kernel::OM->Get('Kernel::System::BWBStore')->StoreList( IncludeInvalid => 0 );
        my $CreatorStores = $Kernel::OM->Get('Kernel::System::BWBAccess')->StoreIDsGet( UserID => $Self->{UserID} );
        if ($CreatorStores) { my %Allowed = map { $_ => 1 } @{$CreatorStores}; $Stores = [ grep { $Allowed{ $_->{StoreID} } } @{$Stores} ]; }
        my %StoreList = map { $_->{StoreID} => $_->{CustomerID} . ' | ' . $_->{StoreNumber} . ' - ' . $_->{StoreName} } @{$Stores};
        my $AssignedStores = ref $Param{BWBStoreIDs} eq 'ARRAY' ? $Param{BWBStoreIDs} : [];
        $Param{BWBStoreOption} = $LayoutObject->BuildSelection(
            Data => \%StoreList, Name => 'BWBStoreIDs', SelectedID => $AssignedStores,
            Multiple => 1, Size => 8, Class => 'Modernize W50pc',
        );
    ...
    $LayoutObject->Block( Name => 'BWBOwnershipFields', Data => \%Param );
...
sub _Overview {
    ...
    if ( !$Kernel::OM->Get('Kernel::System::BWBAccess')->IsGlobalAdministrator( UserID => $Self->{UserID} ) ) {
        my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
        my %Allowed = ( $Self->{UserID} => 1 );
        if ( $DBObject->Prepare(
            SQL => 'SELECT user_id FROM bwb_agent_hierarchy WHERE responsible_user_id = ?',
            Bind => [ \$Self->{UserID} ],
        ) ) {
            while ( my ($UserID) = $DBObject->FetchrowArray() ) { $Allowed{$UserID} = 1; }
        }
        delete $List{$_} for grep { !$Allowed{$_} } keys %List;
        delete $ListAllItems{$_} for grep { !$Allowed{$_} } keys %ListAllItems;
    }
