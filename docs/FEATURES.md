...
- **Localização no Terminar trabalho:** o botão existente pede GPS (Geolocation API) antes do submit. Cadeia: GPS OK → `finish_location_source=gps`; GPS falha → coordenadas da loja do ticket (`bwb_ticket_store` → `bwb_store`) + nota → `source=store`; sem GPS e sem coords da loja → fecha na mesma + nota → `source=none`. Nunca bloqueia o fecho. No **AgentTicketZoom** (só helpdesk), Leaflet + tiles OpenStreetMap injeta o mapa junto ao artigo (o HTML do artigo corre em iframe com CSP que bloqueia scripts/frames externos; imagens estáticas de terceiros falharam). O artigo guarda coordenadas + ligação «Abrir no OpenStreetMap»; o e-mail/portal do cliente não corre este JS. Código: `BWBWorkSession.pm`, `AgentBWBWorkMap.pm`, `Core.Agent.BWBWorkMap.js`, Leaflet 1.9.4 em `htdocs`, XML `BWBWorkMap.xml`. Colunas `finish_*` em `bwb_work_session`. Migração: `db/migrations/2026-08-22-store-work-location.sql`.
