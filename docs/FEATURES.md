...
- **Localização no Terminar trabalho:** o botão existente pede GPS (Geolocation API) antes do submit. Cadeia: GPS OK → `finish_location_source=gps`; GPS falha → coordenadas da loja do ticket (`bwb_ticket_store` → `bwb_store`) + nota → `source=store`; sem GPS e sem coords da loja → fecha na mesma + nota → `source=none`. Nunca bloqueia o fecho. Artigo HTML com mapa estático OSM via `maps.wikimedia.org` (sem API key; pin CSS no centro) e ligação a openstreetmap.org; colunas `finish_latitude` / `finish_longitude` / `finish_accuracy_m` / `finish_location_source` / `finish_location_note` em `bwb_work_session`. Migração: `db/migrations/2026-08-22-store-work-location.sql`.
