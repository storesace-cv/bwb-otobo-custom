...
- **Localização no Terminar trabalho:** o botão existente pede GPS (Geolocation API) antes do submit. Cadeia: GPS OK → `finish_location_source=gps`; GPS falha → coordenadas da loja do ticket (`bwb_ticket_store` → `bwb_store`) + nota → `source=store`; sem GPS e sem coords da loja → fecha na mesma + nota → `source=none`. Nunca bloqueia o fecho. No **AgentTicketZoom** (só helpdesk), secção **Mapa da localização** no fundo do artigo (Leaflet): vista aérea Esri por defeito (alternativa mapa OSM), pin do fecho, e estabelecimentos OSM na área visível via Overpass (lojas, restaurantes/cafés/fast-food, bancos, farmácias, combustível). O artigo em si guarda só coordenadas + ligação OSM; o e-mail/portal do cliente não corre este JS. Código: `BWBWorkSession.pm`, `AgentBWBWorkMap.pm`, `Core.Agent.BWBWorkMap.js`, Leaflet 1.9.4, XML `BWBWorkMap.xml`. Colunas `finish_*` em `bwb_work_session`. Migração: `db/migrations/2026-08-22-store-work-location.sql`.
