...
- **Agendamento na folha (intervenção presencial / «Pendente com Agendamento»):** em vez de campo «Retomar em», a folha abre o **diálogo nativo** de marcação OTOBO (`AgentAppointmentEdit` / `EditMask`) com o ticket pré-ligado (`PluginKey=0100-Ticket`). Botão «Agendar no calendário»; indicador «Marcação registada». Terminar trabalho exige marcação **futura** ligada ao ticket. Sync automático (`BWBAppointmentTicketSync`): ao criar/editar marcação futura, tickets **não encerrados** passam ao estado `Pendente até determinada data` (rótulo **Pendente com Agendamento**) com `Pending till` = início da marcação. **Encerrar o ticket** antes da data cancela automaticamente as marcações futuras ligadas: título `[CANCELADO] …`, motivo na descrição, notificação removida; ficam visíveis no calendário mas deixam de contar como pendentes (dashboard/folha). Guarda `BWBStateRequiresAppointment` impede definir esse estado no zoom sem calendário. Rótulo via `pt_ZZZBWBStates.pm`. Código: `BWBAppointmentCheck.pm`, `BWBAppointmentCancel.pm`, `Ticket/Event/BWBTicketCloseCancelAppointments.pm`, `Core.Agent.BWBWorkAppointment.js`, XML `BWBAppointmentSync.xml`, `BWBWorkAppointment.xml`.
