...
**Modelo `mod-apple-01`:** modelo `Answer` escolhível no Compose **junto** da «Resposta em branco». Visual de cartão (marca Helpdesk → saudação à esquerda → texto editável → caixa resumo com número/estado → botão «Abrir no portal» → rodapé). A saudação **dentro** do cartão replica a «Saudação padrão» da fila (`Olá <OTOBO_CUSTOMER_REALNAME>,` / «Obrigado pelo seu contacto.») mais o espaço para a resposta. O OTOBO 11 concatena sempre a saudação da fila **antes** do modelo (`Ticket::Frontend::ResponseFormat`); o filtro `BWBComposeAppleTemplate` e o JS `Core.Agent.BWBComposeApple.js` retiram esse bloco extra no Compose para não ficar texto solto nem faixa vazia acima do cartão. **Não** se altera a saudação das outras respostas («Resposta em branco»). **Não** é o envelope das notificações (`Default.tt`) nem os e-mails de «Nova ocorrência registada». Um modelo só (BWB e ZS). Tags HTML **obrigatoriamente** encoded (`&lt;OTOBO_CUSTOMER_REALNAME&gt;`, `&lt;OTOBO_TICKET_TicketNumber&gt;`, `&lt;OTOBO_TICKET_State&gt;`, config/URL do portal), como «Exemplo de resposta» e a saudação da fila: o `TemplateGenerator` em RichText só substitui entidades; `<OTOBO_…>` cru é interpretado como tag HTML e some (nome, número, estado e URL vazios). No **PC** o cartão mantém-se (até 580px, agora centrado no fundo cinzento). No **telemóvel** (`max-width: 620px`) o fundo e o cartão passam a 100% da largura disponível (sem `max-width: 580px` nem padding lateral do casco). Tabelas com `width="100%"` para o cliente de correio não encolher o `figure` do CKEditor. Rodapé Helpdesk/StoresAce **sem** «não responda» (é uma resposta). Fonte HTML: `Output/HTML/Templates/Standard/BWBEmail/mod-apple-01.html`. Migração: `db/migrations/2026-08-19-mod-apple-01-answer-template.sql` (filas `bwb-in`, `zsangola-in`, `zs-postmaster`). XML: `BWBComposeApple.xml`.
