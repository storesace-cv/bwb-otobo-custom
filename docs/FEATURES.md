...
**Modelo `mod-apple-01`:** modelo `Answer` escolhível no Compose **junto** da «Resposta em branco». Visual de cartão (marca Helpdesk → saudação → texto editável → caixa resumo com número/estado → botão «Abrir no portal» → rodapé). **Não** é o envelope das notificações (`Default.tt`) nem os e-mails de «Nova ocorrência registada». Um modelo só (BWB e ZS). Tags: `<OTOBO_CUSTOMER_REALNAME>`, `<OTOBO_TICKET_TicketNumber>`, `<OTOBO_TICKET_State>`, ligação `CustomerTicketZoom` com `TicketID`. Rodapé Helpdesk/StoresAce **sem** «não responda» (é uma resposta). Fonte HTML: `Output/HTML/Templates/Standard/BWBEmail/mod-apple-01.html`. Migração: `db/migrations/2026-08-19-mod-apple-01-answer-template.sql` (filas `bwb-in`, `zsangola-in`, `zs-postmaster`).
