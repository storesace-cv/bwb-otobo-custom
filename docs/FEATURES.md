...
- **Agendamento (folha, Responder e Pending):** o mesmo contrato em três sítios. **Folha:** em vez de «Retomar em», diálogo nativo `AgentAppointmentEdit` com ticket pré-ligado (`PluginKey=0100-Ticket`); terminar exige marcação **futura**. **Responder (`AgentTicketCompose`) e Pending (`AgentTicketPending`):** ao escolher «Pendente com Agendamento» (`StateID`), o filtro `BWBComposeAppointment` injeta o bloco «Agendar no calendário»; o JS abre o diálogo sem sair do ecrã, oculta os campos nativos de data pending e bloqueia o submit sem marcação futura. Sync (`BWBAppointmentTicketSync`): marcação futura → estado `Pendente até determinada data` (rótulo **Pendente com Agendamento**) + `Pending till` = início. **Encerrar o ticket** antes da data cancela marcações futuras (`[CANCELADO] …`). Guarda `BWBStateRequiresAppointment` reverte o estado sem calendário. JSON partilhado: `AgentBWBAppointmentCheck`. Código: `BWBAppointmentCheck.pm`, `BWBAppointmentCancel.pm`, `BWBComposeAppointment.pm`, `AgentBWBAppointmentCheck.pm`, `Core.Agent.BWBWorkAppointment.js`, XML `BWBAppointmentSync.xml`, `BWBWorkAppointment.xml`.
...
**Modelo `mod-apple-01`:** modelo `Answer` escolhível no Compose **junto** da «Resposta em branco». Visual de cartão (marca Helpdesk → caixa **Resumo da ocorrência** → saudação numa linha «Olá …, Obrigado pelo seu contacto.» → texto editável → botão «Abrir no portal» cinzento claro → rodapé). Espaçamento no corpo via parágrafos `&nbsp;` (o CKEditor colapsa `margin` em `<p>`). HTML **v2 (2026-08-28):** tabelas simples, estilos **inline**, classe `bwb-answer-card`, **sem** `<figure>`, `<style>`, `!important` nem media queries — optimizado para AgentTicketZoom e clientes de correio. A saudação **dentro** do cartão; o filtro `BWBComposeAppleTemplate` / JS retiram a saudação da fila acima do cartão. Tags HTML **encoded**. Fonte: `BWBEmail/mod-apple-01.html`. Migração histórica de anexos HTML: consola `Maint::BWB::AppleTemplateMigrate` (`Kernel::System::BWBAppleTemplate.pm`) com validação textual antes/depois. Migrações SQL: `2026-08-19-…`, `2026-08-21-*`, `2026-08-28-mod-apple-01-v2-inline.sql`. XML: `BWBComposeApple.xml`. Investigação desempenho: `docs/investigations/mod-apple-01-perf-trace.md`.
...
- **Lembrete «a aguardar resposta do cliente»:** ao entrar no estado `Pendente a aguardar cliente` (folha ou zoom), o evento `853-BWBWaitingCustomerPendingTime` grava `Pending till` = **+3 dias**. O daemon `TicketPendingCheck` dispara `NotificationPendingReminder`; a notificação BD `BWB - Lembrete aguardar resposta cliente` envia e-mail **só ao cliente** (`OncePerDay`, artigo visível no histórico). **Sem** encerramento automático. As notificações nativas de lembrete a agentes (bloqueado/desbloqueado) ficam filtradas aos estados «Pendente com Agendamento» e «Aguardar fornecedor». No HTML do corpo, tags de URL no `href` usam forma encoded (`&lt;OTOBO_CONFIG_…&gt;`). CTA = botão centrado fundo `#59B3FF`, texto branco, cantos arredondados e símbolo Unicode 🔗 (Font Awesome indisponível nos envelopes). Código: `BWBWaitingCustomer.pm`, `Ticket/Event/BWBWaitingCustomerPendingTime.pm`, XML `BWBWaitingCustomer.xml`; migrações `db/migrations/2026-08-26-customer-waiting-reminder*.sql`.
...
- Trabalho aberto: `Output/HTML/Dashboard/BWBOpenWork.pm`, configs em `otobo/Kernel/Config/Files/ZZZBWBDashboard*.pm`. A consulta materializa as linhas SQL **antes** de `TicketAccessCheck` (senão o dashboard só mostrava a primeira folha). Coluna **Loja** a seguir a Cliente (`bwb_ticket_store`). Widgets `TicketGeneric` (novos, **a aguardar resposta do cliente**, abertos, fechados recentes): Cliente + Loja (DF `BWBStore`). Se o agente já tiver colunas personalizadas no widget, pode precisar de activar **Loja** nas definições do próprio widget.
- **A aguardar resposta do cliente:** widget `0129-TicketWaitingCustomer` (módulo nativo `TicketGeneric`), chave **antes** de `0130-TicketOpen`. Filtra o estado `Pendente a aguardar cliente` (o mesmo em que o portal mostra «Fechar ocorrência»). Visibilidade pelas permissões de ticket (`rw` + `BWBAccess`).
- **Ordem dos widgets (causa e correcção):** o núcleo (`AgentDashboardCommon`) usa o prefixo numérico da chave **só** se o agente **não** tiver `UserDashboardPosition`. Com preferência gravada (quase todos os agentes, após qualquer arrasto), widgets **novos** são sempre `push` para o **fundo** da coluna — por isso `0125`/`0126`/`0129` «iam sempre parar ao fundo» independentemente da chave SysConfig. Custom `AgentDashboard.pm` + `BWBDashboard::AgentPositionEnsure`: na carga do painel, chaves `DashboardBackend` em falta na preferência são **inseridas antes do sucessor canónico** (ordem `sort` das chaves), p.ex. `0129` imediatamente antes de `0130`. Não reordena widgets que o agente já tenha posicionado; o arrasto continua a mandar. Manual oficial: [Dashboard (utilizador 11.0)](https://doc.otobo.org/manual/user/11.0/en/content/agent/dashboard/dashboard.html).
...
- Base **NET-bo** interna (produção, 2026-08-25): **Documentação interna → NET-bo** (id **40**, pai id **16**). **17** artigos `NETBO-*` (aprovados, `state_id=2`, ~118 anexos). **Não confundir** com a categoria histórica **Manuais de utilização → NET-bo** (id **10**) — não mover, fundir nem apagar. Corpus versionado: `db/netbo-content/`; SQL gerado por `scripts/apply-netbo-faq.py` (fora do Git); validação `scripts/validate-netbo-faq.py`; export read-only `scripts/export-netbo-faq-from-otobo.sh`. Backup pré-importação: `/root/otobo-backups/faq-before-netbo-internal-20260825.sql`.
