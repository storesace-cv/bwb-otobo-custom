...
O JS `Core.Agent.BWBWorkMap.js` e CSS `BWBWorkMap.css` seguem a matriz `htdocs`. Desde 2026-08-28 o JS **não** faz polling global (`setInterval`); só processa iframes com marcador `.BWBWorkLocation` (mapas de folha). O comando consola `Maint::BWB::AppleTemplateMigrate` e `Kernel/System/BWBAppleTemplate.pm` são consola/CLI — não entram em `runtime-web-system-modules.txt`. O JS `Core.Agent.BWBWorkAppointment.js`, o CSS `BWBWorkAppointment.css` e o CSS nativo `Core.AppointmentCalendar.css` (loader `BWBWorkAppointment.xml` para folha, Compose e Pending) seguem a mesma matriz `htdocs` / SysConfig Loader. O módulo `AgentBWBAppointmentCheck.pm` (JSON de marcação futura) e o filtro `FilterElementPost/BWBComposeAppointment.pm` seguem Modules/Output. O módulo `AgentBWBWorkMap.pm`, o filtro `FilterElementPost/BWBWorkMapEmbedKey.pm` e o XML `BWBWorkMap.xml` / `ZZZBWBWorkMap.pm` seguem Modules/Output e SysConfig compilado. A chave Google Embed (`BWB::MapsEmbedAPIKey`) fica só em SysConfig/produção — nunca no Git.
